...
15. **Abort errors are expected, not failures** — Pi 0.84+ aborts a superseded model refresh; `AbortError`/`"This operation was aborted"` is cancellation, not a provider error. Every provider fetch `catch` block must guard `if (signal?.aborted) return []` *before* logging, so expected aborts never surface as visible `ERROR`/`notify` to the user. When fixing this class of bug (or any cross-cutting convention violation), **examine the whole repo and fix every instance**, not just the reported provider — providers share a near-identical fetch/`catch` shape, so a missing guard in one is a missing guard in all.
