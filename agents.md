...
index.ts                          ← Extension entry point (piFreeEntry)
  ...
  ├─ lib/util.ts                  ← Back-compat re-export shim (new code imports focused modules directly)
  ├─ lib/fetch.ts                 ← Fetch primitives: retry/timeout/deadline/backoff + withSignal (RequestInit omit helper)
  ├─ lib/model-map.ts             ← Size parsing + OpenRouter/OpenAI-compatible mapping
  ...
  │   ├─ settled-decision.ts      ← Pure settled-run decisions: scope predicate, failure classification + abort refinement, auto-continue payload (unit-tested; index.ts orchestrates)
  ...
  └─ providers/                   ← Per-provider extensions (each exports default async fn)
      ...
      ├─ agnes/agnes.ts           ← Agnes AI gateway (flash free, pro paid)
      ├─ commandcode/commandcode.ts ← CommandCode subscription gateway
      ├─ infron/infron.ts         ← Infron AI gateway
      ├─ merge/merge.ts           ← Merge Gateway (keyed catalog)
...
12. **Startup and session-start work are observable and bounded** — legacy `loadCachedOrFetchModels` network attempts use `STARTUP_FETCH_DEADLINE_MS` (8s, override `PI_FREE_STARTUP_FETCH_TIMEOUT_MS`) via `withFetchDeadline` in `lib/fetch.ts` (re-exported via `lib/util.ts`); failed and timed-out attempts are counted with duration and per-provider status. Native providers, including FastRouter, restore from Pi's store and their session-start refresh nudges and detached probes are measured without making intentionally background work block the event. `/free-startup` exposes the post-finalize handler/task timings.
...
18. **Live catalog audits are mandatory before each release and when adding a new provider** — fetch every registered provider's real `/models` endpoint (stored credential from `~/.pi/agent/auth.json` where available; anonymous where the catalog is public) and verify its free/paid classification against pi-free's actual detection semantics (Route A cost detection, Route B name fallback, promotional windows, authoritative `_freeKnown`/`_isFree` stamps) before writing release notes. Record results as a dated entry in `docs/catalog-audit.md` (counts + methodology notes) and present them as a point-in-time snapshot, never a guarantee — README carries no audit table, only a pointer. Providers whose credentials are unavailable are listed as "not audited" rather than guessed. Never print or commit API keys during an audit. **`docs/free_models.md` is a hard release gate**: the dated free-model catalog must be regenerated from the same live audit (model lists + usage conditions per provider) before release notes are written — a stale `free_models.md` blocks the release. A carry-over (no classification/catalog changes) is allowed only with an explicit dated note in both the CHANGELOG entry and the audit doc.
...
- **OpenCode and OpenCode Go** remain Pi-built-in providers; Pi owns their credentials. pi-free captures Pi's catalogs for filtering after session start and then performs one **detached** refresh per tier against OpenCode's public, credential-free `GET /models` endpoint (`/zen/v1/models` for `opencode-free`, `/zen/go/v1/models` for `opencode-go`), so models OpenCode ships between Pi releases appear without waiting for Pi's own pi.dev mirror overlay. Known IDs keep Pi's curated metadata and wire protocol; newly discovered IDs are synthesized through `resolveOpenCodeModelApi`/`applyOpenCodeProtocolDefaults`. The refresh never blocks `session_start` (the handler returns in ~2ms; the fetch lands ~0.6–1.0s later, measured with `PI_STARTUP_BENCHMARK=1`) and is deduplicated per process. A failed **or empty** fetch retains the cached catalog and only logs to `~/.pi/free.log` — it must not rethrow, or Pi surfaces `Could not refresh <provider>; showing cached models` for a transient stall (#504); an aborted refresh (Pi supersedes in-flight refreshes) returns silently per convention 15. Pi's own pi.dev overlay fetch for the built-in `opencode-go` provider runs before ours in Pi's provider composer; its failures are separate from pi-free's direct endpoint fetch. Do not infer a mirror failure from successful endpoint logs: #504 also reproduced with both endpoints returning 200 because pi-free supplied a missing `$OPENCODE_API_KEY` reference. `resolveApiKey` must omit absent credentials rather than install that reference, so Pi treats logged-out providers as unavailable instead of refresh failures.
...
4. **Run the live catalog audit** (convention 18): fetch every registered provider's real endpoint, re-verify free/paid classification, record the dated counts in `docs/catalog-audit.md`, AND regenerate `docs/free_models.md` from the same data — this is a gate: stale free-model documentation blocks the release. A carry-over (no classification/catalog changes) is allowed only with an explicit dated note in both the CHANGELOG entry and the audit doc.
5. **Open a release PR** (`release/X.Y.Z` branch — `master` is branch-protected, direct pushes are refused) and merge it on green; the merge to `master` fires the workflow below.
