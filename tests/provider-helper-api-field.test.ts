 ...
 * silently broke anthropic-protocol providers (openmodel, agentrouter) by
 * forcing their requests to the wrong path (`/v1/chat/completions` returns
 * 404 on anthropic-only gateways). The fix threads an optional `api` field
 * through OpenAICompatibleConfig so each provider can declare its wire
 * format.
 *
...
function makeFakePi(): { registerProvider: ReturnType<typeof vi.fn>; calls: CapturedCall[] } {
...
describe("OpenAICompatibleConfig.api", () => {
	...
	describe("createReRegister", () => {
		it("defaults to openai-completions", () => {
			...
			const reRegister = createReRegister(
				{ registerProvider } as never,
				{ providerId: "p2", baseUrl: "https://x", apiKey: "k2" },
			);
			reRegister([]);
		...
		it("passes through anthropic-messages (openmodel/agentrouter fix)", () => {
			const { registerProvider, calls } = makeFakePi();
			const reRegister = createReRegister(
				{ registerProvider } as never,
				{
	...
	describe("createCtxReRegister", () => {
		it("passes through anthropic-messages when configured", () => {
			...
			const reRegister = createCtxReRegister(
				ctx as never,
				{
					providerId: "openmodel",
					baseUrl: "https://api.openmodel.ai",
