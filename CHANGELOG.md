...
- **pi-ai entries recover from a stale shadowing pi-ai copy** — a long-ago install could leave an old `@earendil-works/pi-ai` in a `node_modules` directory above the pi-free install (correct package name, allowed version, `exports` map predating the allow-listed entries). Node resolved that copy for the loader's bare-specifier fast path and threw `Error: Package subpath './compat' is not defined by "exports" in .../@earendil-works/pi-ai/package.json imported from .../pi-free/dist/lib/pi-ai-loader.js`, which `isPiAiNotFoundError` did not recognize as "pi-ai not found here" — so the disk fallback never ran and every provider stream died at first use on an otherwise-healthy install. The predicate now also accepts `ERR_PACKAGE_PATH_NOT_EXPORTED` for the allow-listed subpaths only (any other subpath is pi-ai's own breakage and still surfaces), and root resolution is **entry-aware**: a candidate copy whose `exports` predates the requested entry is skipped so the search keeps probing toward the running host's own copy instead of pinning a stale one behind the per-entry root cache. Guards: `scripts/smoke-pi-ai-shadowed.mjs` (new CI gate that builds the hostile shadowed layout — no `dist/vendor`, so the vendored last-resort bundle cannot mask a broken disk fallback — and requires sentinel exports from the real copy on every matrix OS), `piAiRootDefinesEntry`/`isPiAiNotFoundError` unit coverage, and recurring-defect-shape #11 in `agents.md` (closes #581).
...
- **CrofAI provider removed** — the provider exposed no free-classified chat models. Dropped from registration, config (`crofai_api_key` / `crofai_show_paid`), `/toggle-crofai`, docs, and package keywords. A stale `crofai` key in `~/.pi/free.json` is ignored.

### Fixed

- **Quieted stale log-stream teardown errors** — rotation and session-transition flushes can report pending writes as `ERR_STREAM_DESTROYED` after the stream has already been detached; those expected errors are now kept out of stderr while active-stream failures still use the synchronous fallback (refs #560).

- **Legacy-host provider bridge auto-detects OMP, never arity-sniffs** — hosts without native Provider registration (Oh My Pi, #543) crashed at startup on the single-arg `registerProvider(provider)` call, so #557 added a two-argument `(id, config)` bridge. Its discriminator sniffed `registerProvider.length`, but stock Pi's extension-facing `registerProvider(providerOrName, config)` overload also has `.length 2`, so the check would have routed the standard host down the legacy path (verified against the installed `pi-coding-agent` bundle; unit mocks with `.length 0` masked it, and the bridge shipped at 0% coverage). The rule is now source-verified: a host exposing `registerNativeProvider` speaks native; otherwise OMP is detected by its own API members (`registerFileWriteFallback`, injected `arktype` shim — both confirmed in `can1357/oh-my-pi` and both absent from the stock bundle), with `oh_my_pi_compat` in `~/.pi/free.json` (or `OH_MY_PI_COMPAT=1`) as a force-legacy override. OMP's `ProviderConfig` also confirms the bridge mapping: `fetchDynamicModels` merges live models over the static snapshot, and its `OAuthLoginCallbacks` carry `onAuth`/`onPrompt`/`onManualCodeInput` but no `onDeviceCode`/`onSelect`, which the bridge tolerates via optional chaining. Stock Pi keeps the native call byte-for-byte. Pinned with `tests/omp-bridge.test.ts` (refs #543).

- **opencode-free compat-registry path still sent the account key** — the anonymous bearer from #544 was applied only to the provider wrapper; the compat fallback registered for the shared `opencode-dynamic` api built a single keyed stream, so auxiliary calls (compaction/title/small) sent the stored credential and Zen answered `429 FreeUsageLimitError` / `403 Model access is disabled` where the identical anonymous request succeeds. The compat registration now builds one anonymous and one keyed stream and dispatches per `model.provider` (`opencode-free` → anonymous, `opencode-go` → keyed) (refs #554).
...
- **OpenCode Go now syncs its live catalog, and a failed catalog refresh no longer surfaces as a Pi warning** — both halves of #504. `opencode-go` was the only built-in-toggle provider without `refreshEndpoint`, so its list was frozen at Pi's build-time snapshot plus Pi's own pi.dev mirror overlay (4h throttle): the public credential-free `GET https://opencode.ai/zen/go/v1/models` endpoint listed 35 models while pi-free showed 27, hiding `glm-5`, `grok-4.5`, `hy3-preview`, `kimi-k2.5`, `mimo-v2-omni`, `mimo-v2-pro`, `minimax-m2.5`, and `qwen3.5-plus` (point-in-time audit, 2026-09-06). The Go tier now runs the same detached post-session-start refresh as `opencode-free` — known IDs keep Pi's curated metadata and wire protocol, newly discovered IDs are synthesized through `resolveOpenCodeModelApi`/`applyOpenCodeProtocolDefaults` (which already special-cases Go's `minimax-*` Anthropic routing) — and it never blocks session start. Separately, Pi's `Could not refresh <provider>; showing cached models` warning was pi-free's own doing for `opencode-free`: the built-in-toggle `refreshModels` rethrew a failed catalog fetch, while every native provider retains the previous list and logs instead (`lib/native-provider.ts`). A transient stall — observed in `~/.pi/free.log` as `opencode-free: endpoint refresh failed {"error":"The operation was aborted due to timeout"}` once the 10s `DEFAULT_FETCH_TIMEOUT_MS` budget elapsed during a network blip — therefore became a user-visible warning even though the cached catalog stayed usable, which is exactly the reported "but opencode itself works". `refreshModels` now retains the cached view on a failed **or empty** fetch (poisoning guard, matching the native-provider recipe) and checks `context.signal.aborted` first so Pi's superseded-refresh aborts stay silent (convention 15); the detached endpoint refresh got the same empty-retain guard. The `opencode-go` half of that warning is Pi-side (its own pi.dev overlay fetch, which the provider composer awaits before ours) and cannot be caught from an extension.
...
- **The test suite no longer leaves a stray `C:/` tree in the repository root** — `tests/health.test.ts` pointed `HOME`/`USERPROFILE` at the literal Windows path `C:/tmp/pi-free-health-test`, which POSIX hosts resolve relative to the cwd, so every `npm run test:run` created an untracked, non-gitignored `C:/tmp/pi-free-health-test/.pi/` (with a generated `free.json` and `free.log`) inside the checkout. It now uses `join(tmpdir(), "pi-free-health-test")`, the same pattern `tests/config-memoization.test.ts` already used; Windows behavior is unchanged.
