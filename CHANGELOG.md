...
- **Legacy-host provider bridge is explicit opt-in, never arity-sniffed** — hosts without native Provider registration (Oh My Pi, #543) crashed at startup on the single-arg `registerProvider(provider)` call, so #557 added a two-argument `(id, config)` bridge. Its discriminator sniffed `registerProvider.length`, but stock Pi's extension-facing `registerProvider(providerOrName, config)` overload also has `.length 2`, so the check would have routed the standard host down the legacy path (verified against the installed `pi-coding-agent` bundle; unit mocks with `.length 0` masked it, and the bridge shipped at 0% coverage). The rule is now explicit: a host exposing `registerNativeProvider` speaks native objects, otherwise the bridge requires `oh_my_pi_compat` in `~/.pi/free.json` (or `OH_MY_PI_COMPAT=1`) — stock Pi keeps the native call byte-for-byte, and OMP users opt in once. Pinned with `tests/omp-bridge.test.ts` (refs #543).
