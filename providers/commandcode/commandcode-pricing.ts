/**
 * CommandCode display pricing (USD PER MILLION tokens).
 *
 * PORTED from patlux/pi-commandcode-provider (`src/pricing.ts`, MIT License,
 * Copyright (c) 2025 Pat Woz) — manually reviewed against CommandCode's
 * official pricing page, last verified there 2026-08-25:
 *   https://commandcode.ai/docs/resources/pricing-limits
 *
 * pi-free converts these to its USD-per-token cost unit at mapping time
 ...
 * amount billed for an individual request. pi-free publishes BASE-tier rates
 * only (its cost model has no tier dimension).
 */
export const MODEL_COSTS: Readonly<Record<string, CommandCodeModelCost>> = {
	...
	// Anthropic — introductory pricing through 2026-08-31 for sonnet-5.
...
/**
 * Time-boxed pricing windows worth re-checking on the source page:
 * introductory/promotional rates that revert when they lapse.
 */

/**
 * Curated per-model metadata tables (ported verbatim from the MIT-licensed
 * patlux/pi-commandcode-provider extension's generated catalog sync,
 * generated from command-code@1.32.2).
 */

export const MODEL_INPUT_MODALITIES: Readonly<Record<string, readonly string[]>> = {
