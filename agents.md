...
index.ts                          ← Extension entry point (piFreeEntry)
  ...
  ├─ lib/util.ts                  ← Back-compat re-export shim (new code: `lib/fetch.ts` for fetchWithRetry/timeout/deadline/retry-backoff, `lib/model-map.ts` for size parsing + OpenRouter/OpenAI-compatible mapping)
