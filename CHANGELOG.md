...
- **Kiro login broken with "Authentication required or access denied." (social relay protocol fix)** — `/login kiro` failed at the very first `InitiateLogin` POST for every fresh-install user. PR #491 had routed ALL IdPs through a localhost callback server by passing `redirect_uri = http://127.0.0.1:<port>/callback` to the Kiro Web Portal's `InitiateLogin`, but the portal's AWS SSO leg (BuilderId — the default IdP) rejects every loopback redirect URI at InitiateLogin time with `401 UnauthorizedException "Authentication required or access denied."` (verified live against `app.kiro.dev` on 2026-09-01; the allowlist also rejects the Kiro-Go reference's `localhost:3128` and every other loopback host/port/path). The Cognito-backed social legs (Google, Github) accepted the InitiateLogin call but then hard-failed the authorize step with `redirect_mismatch` — so no IdP worked. The fix splits capture paths per IdP: **BuilderId** uses the portal's own redirect URI (`app.kiro.dev/signin/oauth`, the only accepted value) with the v2.3.0 manual-paste flow and CBOR `ExchangeToken` — restoring exactly the verified v2.3.0 behavior; **Google/Github** use a new automatic loopback relay (`providers/kiro/kiro-signin-flow.ts`) on the fixed port 3128 — the one loopback URL (`http://localhost:3128/oauth/callback`) that both the portal's `InitiateLogin` and Kiro's Cognito app accept (verified end to end: Cognito 302s to the IdP for it; suffixed and other non-bare loopback variants are `redirect_mismatch`-ed) — with the code exchanged at the IDE's social token endpoint (`prod.us-east-1.auth.desktop.kiro.dev/oauth/token`, JSON `{code, code_verifier, redirect_uri}` → `{accessToken, refreshToken, profileArn, expiresIn}`), the same shape the Kiro IDE itself uses per the portal's web bundle. Both paths verify the CSRF `state`, persist `profileArn` (streaming requirement, PR #485), and fall back to manual paste when the fixed relay port is taken (it cannot be randomized — it is part of Cognito's registered redirect URI).

### Added

- **Auto-fallback to another free model on error** — new `lib/auto-fallback/` module subscribes to `after_provider_response` + `message_end` + `agent_end` and auto-switches to the next-best free model when the current one errors. Active only when `/toggle-free` is **ON** (a deliberate opt-in aligned with the free-mode contract; Pi's own same-model retries still get a chance first via `agent_end.willRetry === false`). Three commands: `/toggle-auto-fallback` (on/off, persists), `/free-fallback-history` (session switch log + blacklist snapshot), `/reset-fallback-blacklist` (escape hatch after exhaustion). The classifier mirrors pi-ai's `isRetryableAssistantError` regex tables locally (convention 16 forbids static pi-ai imports) and adds HTTP-status classification: 429/402/408/425/5xx trigger fallback; 401/403/404/422 are unrecoverable. Selection sorts free candidates by Coding Index score descending, scoped to the failing provider by default (Q3 = D). In-memory blacklist uses a counter + time dual rule (default 10-minute window, 3 strikes → permanent session ban). Notifications aggregate in a 5-minute window to avoid toast spam. `/pi-free-health` adds a one-line `auto_fallback` status. Seven new `~/.pi/free.json` fields (`auto_fallback`, `auto_fallback_scope`, `auto_fallback_providers`, `auto_fallback_blacklist_ttl_ms`, `auto_fallback_blacklist_max`, `fallback_notify`, `fallback_restore`), all optional with sensible defaults — existing configs work unchanged. **Caveats documented in README and AGENTS.md**: Pi does not expose a turn-replay hook (issue earendil-works/pi #1248, `not_planned`), so the failed turn is shown to the user as an error and the *next* turn uses the new model; `setModel()` always rewrites the global default (same issue), with optional `fallback_restore: "auto_next_turn"` to restore the user's pre-fallback pick on recovery.
...
- **Kiro Web Portal auth (kiro `_freeKnown` follow-up) —** the pi-cli OIDC token used by `/login kiro` cannot call `ListAvailableProfiles` (no `codewhisperer:profile:List` scope) so it never receives a `profileArn` from the AWS SSO flow. The Kiro streaming endpoint requires a real `profileArn` and rejects the placeholder with `400 {"message":"Improperly formed request.","reason":"REQUEST_BODY_INVALID"}` (#485), forcing every user with a working setup to set `kiro_profile_arn` in `~/.pi/free.json` by hand. The new `kiro_auth_method: "web-portal"` flow (default for fresh installs) drives a real PKCE + Kiro Web Portal login at `app.kiro.dev/service/KiroWebPortalService/operation/{InitiateLogin,ExchangeToken}` (Smithy `rpc-v2-cbor`), captures the `profileArn` from the `ExchangeToken` response, and persists it to `~/.pi/agent/auth.json` alongside the access token. The streaming path then reads `credential.profileArn` automatically. Users with a pre-existing `kiro_profile_arn` keep the existing `"idc"` flow (migration safety: the default seed is `"idc"` when `kiro_profile_arn` is set, regardless of other config). The refresh path extracts `profileArn` from the `prod.{region}.auth.desktop.kiro.dev/refreshToken` response so subscription upgrades are picked up on the next refresh. The new dep is `cbor-x@^1.6.6` (~50KB, RFC 8949 conformant, the only place it's imported is the cbor wrapper module). Protocol verified end-to-end against the real Kiro Web Portal — see `docs/kiro-web-portal-auth.md` for the full design and `scripts/test-kiro-desktop.mjs` for the manual live test driver.
