...
export default async function (pi: ExtensionAPI) {
  ...
  // Pi resolves apiKey as an env var name lookup. Use a private var so we
  // don't accidentally activate Pi's own built-in opencode provider, which
  // also watches OPENCODE_API_KEY and would create duplicate model entries.
  const ZEN_KEY_VAR = "PI_FREE_ZEN_API_KEY";
  process.env[ZEN_KEY_VAR] = CONFIG_API_KEY ?? "public";
